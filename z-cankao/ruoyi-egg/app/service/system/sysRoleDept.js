...
class SysRoleDeptService extends Service {
  constructor(ctx) {
    super(ctx);
  ...
  async list(params) {
    ...
    let data;
    data = await this.SysRoleDept.findAndCountAll(
  ...
  async delete(roleId) {
    ...
    await this.SysRoleDept.destroy({
      where: {
        role_id: {
          [Op.in]: ids,
        },
  ...
  async update(params, msg) {
    const findRepeat = await this.SysRoleDept.findByPk(params.roleId);
    if (!findRepeat) {
   ...
   */
  async detail(roleId) {
    let data = await this.SysRoleDept.findByPk(roleId);
...
module.exports = SysRoleDeptService;
